...
class EHZipSuite:
    ...
    def crack_menu(self):
        ...
        # ── find best testable file inside ZIP ───────────────────────
        # Rule: prefer DEFLATED (compress_type=8) files so wrong passwords
        # always raise an exception. Stored files (type=0) return garbage
        # silently → CRC check required as safety net regardless.
        try:
            zf_probe = zipfile.ZipFile(zip_path)
            infos = [i for i in zf_probe.infolist()
                     if not i.filename.endswith('/') and i.file_size > 0]
            zf_probe.close()
            if not infos:
                self.err("ZIP contains no usable files.")
                return
            # Prefer compressed entries; fallback to any
            compressed = [i for i in infos if i.compress_type != 0]
            chosen     = min(compressed or infos, key=lambda i: i.compress_size)
            test_file  = chosen.filename
            test_crc   = chosen.CRC          # for CRC-based verification
            self.inf(f"Test file : {test_file}  "
                     f"({'deflated' if chosen.compress_type != 0 else 'stored'})"
                     f"  CRC={test_crc:#010x}")
        except Exception as e:
            self.err(f"ZIP probe error: {e}")
            return
        ...
        # ── auto thread count if default ─────────────────────────────
        import time, os as _os
        cpu = _os.cpu_count() or 4
        if threads_n <= 0:
            threads_n = cpu * 2
        self.inf(f"CPU cores: {cpu}  →  using {threads_n} threads")

        # ── load wordlist into plain list (no Queue lock overhead) ───
        self.inf("Loading wordlist…")
        passwords_list = []
        with open(wl_path, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                w = line.strip()
                if w:
                    passwords_list.append(w)
        ...
        # ── shared found event (faster than bool + lock) ─────────────
        import threading
        found_event   = threading.Event()
        result_lock   = threading.Lock()
        global_count  = [0]          # [0] = shared int via list ref
        count_lock    = threading.Lock()

        start = time.time()

        # ════════════════════════════════════════════════════════════
        # HIGH-SPEED WORKER  — with CRC verification
        # ─ Each thread owns its slice of the list → zero lock on reads
        # ─ CRC checked after every successful read → no false positives
        # ─ Local counter flushed every BATCH → minimal locking
        # ════════════════════════════════════════════════════════════
        import binascii, zlib as _zlib
        BATCH = 200

        def worker(slice_start, slice_end):
            # ── open ZIP once per thread ──────────────────────────
            try:
            ...
            for idx in range(slice_start, slice_end):
                ...
                for enc in ('utf-8', 'latin-1'):
                    ...
                    try:
                        data = zf.read(test_file, pwd=pwd_bytes)

                        # ── CRC verification (critical for stored files) ──
                        actual_crc = binascii.crc32(data) & 0xFFFFFFFF
                        if actual_crc != test_crc:
                            # Garbage data (stored file + wrong password)
                            continue
                    ...
                    except (RuntimeError, _zlib.error):
                        # Wrong password — deflated file raises one of these
                        continue
                    except zipfile.BadZipFile:
                        zf.close()
                        return
                    except Exception:
                        # Any other read error → skip this password
                        continue

                local_count += 1

                # ── flush local count to global every BATCH ───────
                if local_count % BATCH == 0:
                    with count_lock:
                        global_count[0] += BATCH
                        cur = global_count[0]
                    if cur % 2000 == 0:
                        ...
                        pct     = cur / total * 100
                        print(
                            f"{Colors.CYAN}[~] {cur:>9,} / {total:,}"
                            f"  ({pct:5.1f}%)  {rate:,.0f} pwd/s{Colors.END}",
                            end='\r', flush=True,
                        )

            # ── flush remainder ───────────────────────────────────
            remainder = local_count % BATCH
            if remainder:
                with count_lock:
                    global_count[0] += remainder

            zf.close()

        # ── divide list into equal slices, one per thread ────────────
        slice_sz = (total + threads_n - 1) // threads_n
        ts = []
        for i in range(threads_n):
            s = i * slice_sz
            e = min(s + slice_sz, total)
            if s >= total:
                break
            t = Thread(target=worker, args=(s, e), daemon=True)
            ts.append(t)
        ...
        total_tried = global_count[0]
        if self.found:
            self.ok(f"PASSWORD FOUND : {self.password}")
            self.ok(f"Attempts       : {total_tried:,}")
            self.ok(f"Time           : {elapsed:.2f}s")
            rate = total_tried / elapsed if elapsed > 0 else 0
            self.ok(f"Speed          : {rate:,.0f} pwd/s")
        else:
            self.err("Password not found in wordlist.")
            rate = total_tried / elapsed if elapsed > 0 else 0
            self.inf(f"Tried {total_tried:,} passwords in {elapsed:.2f}s  ({rate:,.0f} pwd/s)")
